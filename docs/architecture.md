...
- **Servicio** `js/services/bookingRequestsService.js`: `create` (público), `getAll`, `updateStatus`, `getAvailableTimes` (RPC) y `convertToAppointment` (valida disponibilidad del horario efectivo — permite `overrides.preferredTime`, localiza o crea el paciente, crea la cita CONFIRMADA y marca la solicitud AGENDADA persistiendo la hora final) + helpers `buildWhatsAppUrl`/`buildBookingReminderUrl` (número del consultorio en `WHATSAPP_NUMBER`).
- **Dashboard — operación de solicitudes** (`js/pages/dashboard.js` + `css/dashboard-new.css`): card "Solicitudes de cita" en la primera posición de la columna derecha. Filas enriquecidas (teléfono, antigüedad "llegó hace…", tag Hoy/Próxima, contador "N próx."), clic en la fila abre el detalle (WhatsApp 1-click, Convertir en cita, Marcar contactada, Descartar). Si el horario preferido ya no está libre, un modal muestra los horarios libres del día y permite agendar en 1 clic. **Auto-refresco cada 45 s** con toast de "Nueva solicitud de…" sin recargar, y punto rojo en la campana de notificaciones según pendientes.
- **Recordatorios de WhatsApp sin API** (`database/migrations/011_reminders.sql` + `js/services/remindersService.js`): un job diario (`prepare_reminders()` vía pg_cron/Schedules, SECURITY DEFINER, RLS clínica, sin acceso anon) arma cada noche la cola de recordatorios de las citas CONFIRMADAS de mañana y de las solicitudes PENDIENTE/CONTACTADA con fecha en los próximos 3 días: texto personalizado + enlace `wa.me` con el mensaje ya armado. El dashboard muestra la card "Recordatorios de hoy" y un modal con la cola completa (enviar con 1 tap, "marcar enviado", "saltar") + botón "Generar" que corre `prepare_reminders()` a demanda. El envío último sigue siendo manual (1 tap) porque WhatsApp bloquea el envío automatizado desde cuentas personales; la automatización cubre decidir, redactar y clasificar.
